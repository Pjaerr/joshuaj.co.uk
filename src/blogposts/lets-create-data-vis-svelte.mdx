...
image: "/lets-create-data-vis-svelte/overview-component.jpg"
---

import Video from "../components/blog/Video/Video";
...
<Video
  src="/lets-create-data-vis-svelte/final-app"
...
![Colour Palette](/lets-create-data-vis-svelte/colour-palette)
...
<Video
  src="/lets-create-data-vis-svelte/basic-draw-transition"
...
true)} class="path" d="{svgPath}" fill="{fillColour}" stroke="{strokeColour}"
style="stroke-width: {strokeWidth}" />
```
...
```html
<path fill={transitionEnded ? fillColour : strokeColour} stroke={transitionEnded
...
<Video
  src="/lets-create-data-vis-svelte/finished-draw-transition"
...
```html
<path on:click transition:draw={{ duration: 1500 }} on:introend={ () =>
(transitionEnded = true)} d={svgPath} class="path" fill={transitionEnded ?
fillColour : strokeColour} stroke={transitionEnded ? strokeColour : fillColour}
style="stroke-width: {strokeWidth}" />
```
...
```html
<MapContainer>
  {#each regions as { name, svgPath }} <MapRegion on:click={() => {
  console.log(name + ' clicked!') }} {svgPath} fillColour="red"
  strokeColour="white" strokeWidth="1px" /> {/each}
</MapContainer>
```
...
<main class="app">
  ...
  <div class="map-container">
    <MapContainer>
      {#each Regions as { name, svgPath }} <MapRegion on:click={() => {
      activeRegion = name; }} {svgPath} fillColour="red" strokeColour="white"
      strokeWidth="1px" /> {/each}
    </MapContainer>
  </div>
</main>
```
...
<Video
  src="/lets-create-data-vis-svelte/interactive-map"
...
![Example of RegionInformation component on the page](/lets-create-data-vis-svelte/info-for-derbyshire)
...
const players = region.players.sort((player1, player2) => {
  ...
  player1.seasons.forEach(season => (player1Appearances += season.appearances));
  player2.seasons.forEach(season => (player2Appearances += season.appearances));
...
![Example of RegionInformation component on the page as a modal](/lets-create-data-vis-svelte/info-for-derbyshire-modal)
...
```html
{#if activeRegion} <RegionInformation region={getRegionData(activeRegion)}
onClose={() => { activeRegion = undefined; }} /> {/if}
```
...
<Video
  src="/lets-create-data-vis-svelte/regioninformation-modal"
...
```html
{#if activeRegion} <RegionInformation region={getRegionData(activeRegion)}
onClose={() => { activeRegion = undefined; }} /> {:else}
<Overview />
{/if}
```
...
![Overview component](/lets-create-data-vis-svelte/overview-component)
