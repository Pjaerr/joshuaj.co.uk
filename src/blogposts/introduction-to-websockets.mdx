...
description: "In this blogpost I will be covering the basics of websockets by creating a website using HTMLm CSS, JavaScript and Node.js that allows users to paint on a canvas with other users in realtime."
issueLink: "Pjaerr/Personal-Website/issues/6"
image: "/introduction-to-websockets/cover_image.jpg"
---

import Video from "../components/mdx/Video/Video";

_This guide is a brief introduction to websockets and is a precursor to my upcoming tutorial on making a multiplayer quiz game using react and websockets._
...
Once you've installed node, you'll probably need to restart your terminal <small>(You can check that node is installed by running `node -v`)</small>.
...
We also store the colour that we've chosen inside of a variable called `thisColour`. This is important for when we implement websockets as we will be drawing other users
...
```javascript{18,20,21,23,24,25,26,27,28}:title=client/main.js
const canvas = document.querySelector("canvas");
const ctx = canvas.getContext("2d");
...
Visit `http://localhost:8080`, refresh the page, and then click anywhere on the page. If you look at the terminal that is running your server <small>(the one you ran `node main.js` in)</small>, you should see "Client is clicking their mouse!" being printed out.
...
We can do this easily as, when a websocket disconnects, it sends a "close" message to the server. Let's use this to remove it from the array.
...
This may or may not be your desired functionality, but a possible solution would be to store a queue of actions on the server and, every time a client sends a message, add it to the queue. Then, when a new/rejoining client connects, send them the queue so that they can draw it on their canvas but I'll leave it up to you to implement a solution.
...
I will be publishing another blogpost in the near future that guides you through **Creating a Multiplayer Quiz Game using React and Websockets** which will delve further into websockets and cover some cool stuff such as:

- Implementing a room system so that users can create and join rooms using a code

- Basic cheat prevention

- Maintaining game state across connections
